...
const InterfaceEdit = () => {
  ...
  const [existingInterface, setExistingInterface] = useState<InterfaceRecord | null>(null);
  const [createPrefill, setCreatePrefill] = useState<InterfacePrefill | null>(null);
  ...
  useEffect(() => {
    ...
    const sysName = existingInterface.sys_name ?? '';
    const knownNames = systemInterfaces.map((item) => item.sys_name);
    const selectorValue = sysName && knownNames.includes(sysName) ? sysName : OTHER_OPTION;

    form.setFieldsValue({
      name: existingInterface.name ?? '',
      ip: existingInterface.ip ?? '',
      sys_name: sysName,
      enabled: existingInterface.enabled ?? true,
  ...
  const fetchSystemInterfaces = async () => {
    try {
      ...
      messageApi.error(`Failed to fetch system interfaces: ${getErrorMessage(error, 'Failed to fetch system interfaces')}`);
  ...
  const fetchInterface = async () => {
    try {
      setLoading(true);
      const result = await interfacesApi.getById(String(id));
      setExistingInterface(result.data);
    } catch (error) {
      messageApi.error(`Failed to fetch interface data: ${getErrorMessage(error, 'Failed to fetch interface data')}`);
      console.error('Error:', error);
    } finally {
      setLoading(false);
    }
  ...
  const handleSysNameSelectorChange = (value: string) => {
  ...
  const handleSave = () => {
    form.validateFields()
      .then(async (values: InterfaceFormValues) => {
        const sysName =
        ...
        const payload = {
          name: values.name,
          sys_name: sysName,
          enabled: values.enabled ?? existingInterface?.enabled ?? true,
          ip:
        ...
        try {
          if (id === 'new') {
            ...
            await interfacesApi.update(String(id), payload);
          }
          ...
          loadingMessage();
          messageApi.error(`Failed to save interface: ${getErrorMessage(error, 'Failed to save interface')}`);
          console.error('Error:', error);
        }
      })
      .catch(() => {
            ...
            <Col style={{ width: '100%', maxWidth: '1000px' }}>
              <Card title="Interface Options" size="small" loading={loading}>
                ...
                <Form.Item noStyle dependencies={['sys_name_selector']}>
                  {({ getFieldValue }: FormInstance<InterfaceFormValues>) =>
