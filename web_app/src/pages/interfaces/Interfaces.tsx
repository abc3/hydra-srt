...
import type { InterfaceRow, SystemInterface } from '../../types/interfaces';
...
const Interfaces = () => {
  ...
  const fetchInterfaces = async () => {
    try {
      ...
      const saved: SystemInterface[] = Array.isArray(savedResult.data) ? savedResult.data : [];
      const system: SystemInterface[] = Array.isArray(systemResult.data) ? systemResult.data : [];

      const savedBySysName = saved.reduce<Record<string, SystemInterface>>((acc, item) => {
        if (item?.sys_name) {
      ...
      const systemRows: InterfaceRow[] = system.map((item) => {
        ...
        return {
          ...
          ip: item.ip || '',
          multicast_supported: item.multicast_supported ?? false,
      ...
      const missingSavedRows: InterfaceRow[] = saved
        .filter((item) => !system.some((systemItem) => systemItem.sys_name === item.sys_name))
        .map((item) => ({
          key: item.id || `custom:${item.sys_name}`,
          id: item.id || null,
          name: item.name || '',
          sys_name: item.sys_name,
          ip: item.ip || '',
      ...
      messageApi.error(`Failed to fetch interfaces: ${getErrorMessage(error, 'Failed to fetch interfaces')}`);
  ...
  const openRawInfoDrawer = async () => {
    try {
      ...
      messageApi.error(`Failed to fetch raw interface info: ${getErrorMessage(error, 'Failed to fetch raw interface info')}`);
      console.error('Error:', error);
    } finally {
  ...
  const startEdit = (record: InterfaceRow) => {
  ...
  const saveName = async (record: InterfaceRow, options: SaveOptions = {}) => {
    ...
    try {
      ...
      if (!targetId) {
        ...
        const existingRows: SystemInterface[] = Array.isArray(existing?.data) ? existing.data : [];
        const existingRow = existingRows.find((item) => item?.sys_name === record.sys_name);
      ...
      messageApi.error(`Failed to save interface name: ${getErrorMessage(error, 'Failed to save interface name')}`);
      console.error('Error:', error);
      await fetchInterfaces();
    } finally {
      setSavingKey('');
  ...
  const toggleEnabled = async (record: InterfaceRow, enabled: boolean) => {
    try {
      ...
      messageApi.error(`Failed to update interface visibility: ${getErrorMessage(error, 'Failed to update interface visibility')}`);
      console.error('Error:', error);
    } finally {
      setTogglingKey('');
    }
