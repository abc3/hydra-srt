...
const formatChartTimestamp = (value: string | undefined, includeSeconds = false): string => {
  if (!value) return '';
...
const formatBitrate = (bytesPerSec: number | undefined | null): string => {
...
const formatRoundedMetricValue = (value: unknown): unknown => {
  if (typeof value !== 'number' || Number.isNaN(value)) return value;
  return Math.round(value);
};

const calcAverage = (values: unknown[]): number | null => {
  const nums = values.filter((v): v is number => typeof v === 'number' && !Number.isNaN(v));
  if (!nums.length) return null;
  return nums.reduce((sum, v) => sum + v, 0) / nums.length;
};

const formatOneDecimal = (value: number | null | undefined): string => {
...
const renderChartTooltip = ({ active, payload }: { active?: boolean; payload?: readonly AnyRecord[] }) => {
  ...
  const formatTooltipValue = (entry: AnyRecord) => {
...
const alignTsToBucket = (tsMs: number, bucketMs: number): number => Math.floor(tsMs / bucketMs) * bucketMs;
...
const SystemNodeMetrics = () => {
  ...
  const [customRangeDraft, setCustomRangeDraft] = useState<[dayjs.Dayjs | null, dayjs.Dayjs | null]>([
    dayjs().subtract(1, 'hour'),
    dayjs(),
  ]);
  const [customRangeApplied, setCustomRangeApplied] = useState<[dayjs.Dayjs | null, dayjs.Dayjs | null]>([
    dayjs().subtract(1, 'hour'),
    dayjs(),
  ]);
  const [analyticsData, setAnalyticsData] = useState<AnalyticsState>({ points: [], meta: null });
  ...
  useEffect(() => {
    ...
    const loadRouteVisibleInterfaces = async () => {
      try {
        ...
        const savedBySysName = saved.reduce((acc: Record<string, any>, item: AnyRecord) => {
          if (item?.sys_name) {
        ...
        const mergedRows = [
          ...
          ...saved
                    .filter((item: AnyRecord) => !system.some((systemItem: AnyRecord) => systemItem.sys_name === item.sys_name))
                    .map((item: AnyRecord) => ({
              sys_name: item.sys_name,
  ...
  const fetchAnalyticsData = useCallback(async (queryParams: QueryParams) => {
    if (!nodeId) return;
  ...
  const chartData = useMemo(() => {
    ...
    const normalizePoint = (point: AnyRecord): AnyRecord => {
      const nextPoint: AnyRecord = {
        ...point,
    ...
    const pointsByBucket = new Map<string, AnyRecord>();
    rawPoints.forEach((point: AnyRecord) => {
  ...
  const networkSeries = useMemo(() => {
    ...
    chartData.forEach((point: AnyRecord) => {
      Object.keys(point).forEach((key: string) => {
  ...
  const networkDomain = useMemo(() => {
    const values: number[] = [];
    networkChartData.forEach((point: AnyRecord) => {
      renderedNetworkSeries.forEach((key: string) => {
                  ...
                  <ResponsiveContainer>
                    <LineChart data={networkChartData} margin={{ top: 8, right: 20, left: 8, bottom: 8 }}>
                      ...
                      <YAxis
                        ...
                        tickFormatter={(value: number) => formatSignedThroughput(value)}
                      />
                      <RechartsTooltip content={renderChartTooltip as any} />
