...
const SystemNodes = () => {
  ...
  const getProgressColor = (value: number | null | undefined): string => {
  ...
  const formatThroughput = (bytesPerSec: number | null | undefined): string => {
  ...
  const formatHostOnly = (value: unknown): string => {
    if (!value) return 'N/A';
  ...
  const columns: ColumnsType<SystemNode> = [
    ...
    },
    {
      ...
      sorter: (a: SystemNode, b: SystemNode) => {
    ...
    },
    {
      ...
      key: 'network',
      render: (_: unknown, record: SystemNode) => (
        <Space size={12}>
