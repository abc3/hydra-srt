defmodule HydraSrt.Stats.MetricSelector do
  ...
  def select_rows(%{route_id: route_id, stats: stats} = envelope)
    ...
    source_rows = source_bytes_in_rows(route_id, stats, ts)
  ...
  @spec source_bytes_in_rows(binary(), map(), NaiveDateTime.t() | DateTime.t()) :: [map()]
  def source_bytes_in_rows(route_id, stats, ts)
      when is_binary(route_id) and is_map(stats) do
